...
contract LiquidationEngine {
    ...
    event LiquidationExecuted(address indexed vault, address indexed vaultOwner, uint256 bnbAmount, uint256 usdtRecovered, uint256 protocolFee);
    ...
    modifier onlyKeeper() { if (!authorizedKeepers[msg.sender] && msg.sender != owner) revert NotKeeper(); _; }
    ...
    function checkAndLiquidate(address vaultAddress) external onlyKeeper {
        ...
        liquidationHistory.push(LiquidationRecord({
            vault: vaultAddress, vaultOwner: vaultOwner,
            bnbLiquidated: bnbReceived, usdtRecovered: usdtReceived,
    ...
    function _swapBNBtoUSDT(uint256 bnbAmount, uint256 debtUSDCents, address vaultOwner) internal returns (uint256 usdtReceived) {
        uint256 minUSDTOut = (debtUSDCents * 1e6 / 100);
        minUSDTOut = (minUSDTOut * (BPS_DENOMINATOR - SLIPPAGE_BPS)) / BPS_DENOMINATOR;
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = USDT;
        uint[] memory swapAmounts = IPancakeRouter(pancakeRouter)
            .swapExactETHForTokens{value: bnbAmount}(minUSDTOut, path, address(this), block.timestamp + 15 minutes);
        ...
        if (surplus > 0) { IERC20(USDT).transfer(vaultOwner, surplus); emit SurplusReturned(vaultOwner, surplus); }
    }

    function authorizeKeeper(address keeper) external onlyOwner { authorizedKeepers[keeper] = true; emit KeeperAuthorized(keeper); }
    ...
    function transferOwnership(address newOwner) external onlyOwner { if (newOwner == address(0)) revert ZeroAddress(); owner = newOwner; }
    function getLiquidationCount() external view returns (uint256) { return liquidationHistory.length; }
